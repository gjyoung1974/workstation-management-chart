...
```bash
$ helm install --name sal-postgresql postgresql
```
...
To uninstall/delete the `sal-postgresql` deployment:

```bash
$ helm delete sal-postgresql
```
...
```bash
$ helm install --name sal-postgresql \
  --set postgresUser=my-user,postgresPassword=secretpassword,postgresDatabase=my-database postgresql
```
